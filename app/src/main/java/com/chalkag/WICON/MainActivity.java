...
public class MainActivity extends AppCompatActivity {
    ...
    private int SSID_INDEX=0;
    private boolean PW_STATE=false;
...
//        wifi.setOnClickListener(view -> connect_ap(ssid)) ;
    ...
    private static String convertResponseToString(BatchAnnotateImagesResponse response) {
        ...
        System.out.print("OCR RESULT:");
        System.out.println(message);


        StringBuffer id = new StringBuffer(message.toString().replace("\n", " ").replace(":", ""));

        String BUF = id.toString();
        ocr_result = BUF.split(" ");
        for (int i = 0; i < ocr_result.length; i++) {
            if (ocr_result[i].isEmpty()) ocr_result[i] = "null";
            System.out.println(ocr_result[i]);
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                Toast toast = Toast.makeText(dd, "OCR 완료", Toast.LENGTH_SHORT);
                toast.setGravity(Gravity.BOTTOM, 0, 350);
                toast.show();
            }
        }, 0);
    ...
    private boolean connect_ap(String ssid) {
        ...
        for(int i=0; i <WIFI.size(); i++){
            if(state==true)
                break;
            System.out.print("WIFI.SSID:");
            System.out.println(WIFI.get(i).SSID);
            for(int j=0;j<ocr_result.length;j++){
                System.out.print("ocr_result:");
                System.out.println(ocr_result[j]);
                if(WIFI.get(i).SSID.equals(ocr_result[j])) {
                    wifiConfig.SSID = "\"" + ocr_result[j] + "\"";
                    ssid = ocr_result[j];
                    System.out.println("=============================================================");
                    System.out.println("=============================================================");
                    SSID_INDEX=j;
        ...
        if(state) {
            if (network.contains("open")) {
                ...
                for(int k=SSID_INDEX+1; k<ocr_result.length; k++)
                {
                    System.out.print("connect wifi::::::");
                    System.out.println(ocr_result[k]);
                    wifiConfig.preSharedKey = "\"".concat(ocr_result[k]).concat("\"");

                    if(connect_Wifi(wifiConfig)!= -1 ){
                        PW_STATE = true;
                        break;
                    }
                    else{
                ...
                for(int k=SSID_INDEX+1; k<ocr_result.length; k++)
                {
                    wifiConfig.wepKeys[0] = ocr_result[k];
...
//            wifiManager.reconnect();
    ...
    int connect_Wifi(WifiConfiguration wifiConfig){
        wifiManager = (WifiManager) getApplicationContext().getSystemService(Context.WIFI_SERVICE);
